    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 5; j++) {
            if (i == 0 || i == 3) {
                ...
                asiento.ocupado = 0; // Por defecto, el asiento está libre
                impar += 2; // Incrementar por 2 para obtener el siguiente número impar
            } else {
                asiento.lugar = 'p';
                asiento.numero = par;
                asiento.ocupado = 0; // Por defecto, el asiento está libre
                par += 2;   // Incrementar por 2 para obtener el siguiente número par
            }
...
void comprar_asiento(){
    ...
    while (!feof(archivo)) {
        if (numero == asiento.numero) {
            printf("Numero: %d, Lugar: %c, Estado: %s\n", asiento.numero, asiento.lugar, asiento.ocupado);
...
void liberar_asiento() {
    ...
    if (!existe) {
        printf("No existe un asiento con ese numero o ya está libre.\n");
    }
